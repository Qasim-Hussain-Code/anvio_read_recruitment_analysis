...
The reference genome (`genome.fa`) is a single-contig assembly — approximately 5.1 Mb in length — representing a microbial population of interest. The metagenomic datasets comprise paired-end Illumina-style reads (R1/R2) for each of the five mock samples. Although the data are synthetic, the analytical steps mirror real-world workflows used to study the distribution of microbial populations across human-associated and environmental metagenomes.
...
1. **Does every individual carry a microbial population that matches this genome?** — Answered by examining mean coverage and detection statistics across all five metagenomes at the contig level.

2. **Does every gene in this genome occur in every individual?** — Answered by switching to gene-mode visualisation, which reveals that certain genes — despite belonging to the same genome — are absent or undetected in specific individuals. This observation reflects the biological reality that accessory genes within a single genome may not be uniformly present across populations in different hosts.
...
> **Note** — Directories containing sequencing data, alignments, and database files exceed 100 MB per file and are excluded from version control. Placeholder `README.md` files are committed within each data directory to preserve the full project structure on GitHub. See the _Data Availability_ section below for retrieval instructions.
...
|------------|------------------------------------------------|---------------------|---------------------|
| Alejandra  | Mock human gut metagenome — Alejandra          | alejandra-R1.fastq  | alejandra-R2.fastq  |
| Batuhan    | Mock human gut metagenome — Batuhan            | batuhan-R1.fastq    | batuhan-R2.fastq    |
| Jessika    | Mock human gut metagenome — Jessika            | jessika-R1.fastq    | jessika-R2.fastq    |
| Jonas      | Mock human gut metagenome — Jonas              | jonas-R1.fastq      | jonas-R2.fastq      |
| Magdalena  | Mock human gut metagenome — Magdalena          | magdalena-R1.fastq  | magdalena-R2.fastq  |
